...
function getMonthlyData(bookings: Booking[]) {
  ...
  return months.map((month) => ({
    name: month.label,
    bookings: counts[month.key] ?? 0,
  }))
}

export default function BookingChart({ bookings }: { bookings: Booking[] }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-foreground">Bookings this year</p>
