...
import { getAdminUserFromSession, getAllVolunteerApplications } from '@/lib/admin'
...
export default async function AdminVolunteersPage() {
  ...
  const volunteers = await getAllVolunteerApplications()
    ...
    <AdminShell user={user}>
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card">
        <table className="w-full min-w-[820px] table-auto">
          ...
          <tbody>
            {volunteers.map((vol) => (
              <tr key={vol.id} className="border-t border-border">
                <td className="px-4 py-3 text-sm">{vol.name}</td>
                <td className="px-4 py-3 text-sm">{vol.email}</td>
                <td className="px-4 py-3 text-sm">{vol.phone}</td>
